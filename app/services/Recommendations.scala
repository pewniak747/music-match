...
class Recommendations(val request: RecommendationRequest) {
  ...
  lazy val similarArtistsQuery = SQL("SELECT DISTINCT artists.id FROM artists INNER JOIN taggings ON artists.id = taggings.artist_id WHERE taggings.tag_id IN (%s) AND artists.id NOT IN (%s)".format(favouriteTagIds.mkString(","), favouriteArtistIds.mkString(",")))
