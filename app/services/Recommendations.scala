...
class Recommendations(val request: RecommendationRequest) {
  ...
  def popularFromSimilarArtists: Seq[Long] = DB.withConnection { implicit c => popularFromSimilarArtistsQuery().map { row => row[Long]("song_id") }.toList }
  ...
  lazy val popularFromSimilarArtistsQuery = SQL("SELECT songs.artist_id, MAX(songs.id) as song_id, count(scrobbles) FROM songs LEFT OUTER JOIN scrobbles ON scrobbles.song_id = songs.id WHERE artist_id IN (%s) AND songs.id NOT IN (%s) GROUP BY songs.artist_id ORDER BY count(scrobbles) LIMIT 7".format(similarArtistIds.mkString(","), alreadySeenSongIds.mkString(",")))
